...
describe("chartUtils", () => {
  ...
  describe("getResponsiveChartHeight", () => {
    it("should return mobile height for small screens", () => {
      ...
      expect(getResponsiveChartHeight(CHART_BREAKPOINTS.mobile - 1)).toBe(
        CHART_HEIGHTS.mobile,
      );
    });

    it("should return tablet height for medium screens", () => {
      expect(getResponsiveChartHeight(CHART_BREAKPOINTS.mobile)).toBe(
        CHART_HEIGHTS.tablet,
      );
      expect(getResponsiveChartHeight(700)).toBe(CHART_HEIGHTS.tablet);
      expect(getResponsiveChartHeight(CHART_BREAKPOINTS.tablet - 1)).toBe(
        CHART_HEIGHTS.tablet,
      );
    });

    it("should return desktop height for large screens", () => {
      expect(getResponsiveChartHeight(CHART_BREAKPOINTS.desktop)).toBe(
    ...
    it("should handle exact breakpoint values", () => {
      ...
      expect(getResponsiveChartHeight(CHART_BREAKPOINTS.tablet)).toBe(
        CHART_HEIGHTS.desktop,
      );
    });
  });
